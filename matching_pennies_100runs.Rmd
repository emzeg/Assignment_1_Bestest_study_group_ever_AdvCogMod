...
```{r}
pacman::p_load(tidyverse, ggplot2, tibble, ggthemes, gt)
set.seed(1234)
...
#plot of winrates
big_plot <- ggplot(results_long, aes(x = trial, y = cumwinrate, color = player)) +
  ...
  labs(
    ...
    scale_color_manual(
    values = c("mean_p1_cumwinrate" = "lightpink", "mean_p2_cumwinrate" = "lightgreen"),
...
results_mean %>%
  ...
  opt_table_font(
    font = list(
      google_font(name = 'serif')
    )) %>%
  tab_header(title = md("Percentage of Cumulative Win Rate"))
