...
plot_winrate_f <- function(df) {
  ...
  return (winrate_plot) #not returning the df for now? (though changing it)
}
...
print(sum(df$feedback)) #how often player 2 wins (WSLS)
```
...
ggplot(df1) +
  ...
  theme_tufte() + 
  labs(x = "Trials", y = "Win", fill = "Variable", title = "Random Choice vs Strategy Switcher") +
  scale_fill_manual(values = c("randomChoice" = "#7df3e1", "StratSwitcherChoice_against_Random" = "lightgreen")) +
...
print(sum(df1$feedback)) #how often player 2 wins (StratSwitcher)
```
```{r}
...
ggplot(df2) +
  ...
  geom_bar(aes(x = trials, y = -StratSwitcherChoice_against_Random, fill = "StratSwitcherChoice_against_Random"), stat = "identity") +
  theme_tufte() + 
  labs(x = "Trials", y = "Win", fill = "Variable", title = "Normal WSLS vs Strategy Switcher") +
  scale_fill_manual(values = c("WSLSChoice" = "#FFC067", "StratSwitcherChoice_against_Random" = "lightgreen")) +
  scale_y_continuous(limits = c(-1, 1), breaks = c(-1, 0, 1), labels = c("1", "0", "1")) +
  theme(
    plot.title = element_text(size = 16),
    axis.title = element_text(size = 14),
...
print(sum(df2$feedback)) #how often player 2 wins (StratSwitcher)
```
...
print(sum(df1_2$feedback)) #how often player 2 wins (Modified WSLS)
```
...
print(sum(df2_2$feedback)) #how often player 2 wins (Modified WSLS) (WORSE than normal WSLS)
```
